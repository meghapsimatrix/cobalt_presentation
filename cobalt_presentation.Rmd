...
The CRAN version does not work with tibbles but the development version does (but then major functions don't run). Here, we are using the CRAN version.
...
If you use `readr` or `dplyr`, make sure to convert the tibble to a dataframe if using the CRAN version of `cobalt`.

```{r warning = FALSE, message = FALSE}
Algebra_dat <-
  read_csv("datasets/8th-grade-Algebra-data.csv") %>%
  mutate(Locale = factor(Locale, levels = c("R","S","U"), labels = c("Rural","Suburban","Urban")))

# convert to data frame
Algebra_dat <- Algebra_dat_org <- data.frame(Algebra_dat)
```
...
```{r fig.width = 7, fig.height = 4.5, dpi = 500, eval = FALSE}
...
```{r fig.width = 7, fig.height = 4.5, dpi = 500, echo = FALSE}
love.plot(b_w1, threshold = .1, colors = c("red", "blue"),
          size = 1.5)
```
...
```{r fig.width = 7, fig.height = 4.5, dpi = 500, eval = FALSE}
love.plot(b_w1, threshold = 2, colors = c("red", "blue"), 
          size = 1.5, stat = "variance.ratios")
```
...
```{r fig.width = 7, fig.height = 4.5, dpi = 500, echo = FALSE, warning = FALSE, echo = FALSE}
bal.plot(f_lin, data = Algebra_dat, weights = "att_wt", 
         method = "weighting", estimand = "ATT",
         var.name = "SES", which = "both") +
  scale_fill_manual(values = c("red", "blue")) +
...
```{r fig.width = 7, fig.height = 4.5, dpi = 500, echo = FALSE, warning = FALSE, message= FALSE}
bal.plot(f_lin, data = Algebra_dat, weights = "att_wt", 
         method = "weighting", estimand = "ATT",
         var.name = "Locale", which = "both") +
  scale_fill_manual(values = c("red", "blue")) +
...
Balance should be evaluated for two-way interactions and squared terms of continuous covariates (Stuart, 2010). We can add `int = TRUE` as an argument. 


```{r eval = FALSE}
(b_w2 <- bal.tab(f_lin, data = Algebra_dat, weights = "att_wt", 
         method = "weighting", int = TRUE, 
...
```{r echo = FALSE}
(bal.tab(f_lin, data = Algebra_dat, subclass = "quintile", 
        method = "subclassification", estimand = "ATT", 
        disp.v.ratio = TRUE, un = TRUE))
```


__Does not show variance ratios across subclasses.__ 
...
bal.plot(f_lin, data = Algebra_dat, 
         weights = data.frame(bad = get.w(m_out),
                              better = get.w(better_match)),
         method = "weighting", var.name = "SES", which = "both") +
  scale_fill_manual(values = c("red", "blue")) +
  theme_minimal()
```
