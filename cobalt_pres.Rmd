...
```{r fig.width = 7, fig.height = 4.5, dpi = 500}
...
bal.plot(f_lin, data = Algebra_dat, weights = "att_wt", 
         method = "weighting", estimand = "ATT",
         var.name = "Locale", which = "both", alpha = .2, position = "stack") + # alpha doesn't seem to work? and position doesn't work
  scale_fill_manual(values = c("red", "blue")) +
  theme_bw()
```
...
```{r fig.width = 6.5, fig.height = 4, dpi = 500, error = TRUE}
love.plot(b_s1)
```
...
bal.plot(f_lin, data = Algebra_dat, 
         weights = data.frame(bad = get.w(m_out),
                              better = get.w(better_match)),
         method = "weighting", var.name = "SES", which = "both", alpha = .2) +
  scale_fill_manual(values = c("red", "blue")) +
  theme_minimal()
```
